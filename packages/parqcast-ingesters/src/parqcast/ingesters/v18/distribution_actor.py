...
class DistributionActorV18(BaseIngester[V18]):
    """Create inter-warehouse stock pickings from planning decisions.

    ``stock.move``'s UoM field is ``product_uom`` in both v18 and v19
    (the rename story is for ``purchase.order.line`` / ``sale.order.line``
    — stock_move was never touched). Create surface is identical to v19.
    """
