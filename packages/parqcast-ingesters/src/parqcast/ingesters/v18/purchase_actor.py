...
class PurchaseActorV18(BaseIngester[V18]):
    """Create purchase orders and lines from planning-engine decisions (v18).

    On v18 the UoM field on ``purchase.order.line`` is ``product_uom`` (it
    became ``product_uom_id`` in v19 — see docs/odoo-18-vs-19-evidence.md
    §2). Otherwise the create surface is unchanged.
    """
