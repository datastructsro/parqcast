...
class StockLotCollectorV18(StockCollector[V18]):
    """Stock-lot collector for Odoo 18.

    Expiry columns (``expiration_date`` / ``use_date`` / ``removal_date`` /
    ``alert_date``) come from the ``product_expiry`` module on both v18 and
    v19 — gated via ``optional_columns`` so a vanilla install without
    product_expiry still populates the row with NULLs."""
