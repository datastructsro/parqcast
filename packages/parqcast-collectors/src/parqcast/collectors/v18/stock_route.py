...
class StockRouteCollectorV18(StockCollector[V18]):
    """Denormalised route + rules. ``stock_route.name`` and ``stock_rule.name``
    are both translatable Char stored as JSONB on v18 (same as v19).

    ``stock_route_warehouse`` is the m2m link table; column surface stable
    across both majors.
    """
